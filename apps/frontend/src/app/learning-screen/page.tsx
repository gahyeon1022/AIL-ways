...
export default function LearningScreenPage() {
  ...
  useEffect(() => {
    ...
    const upload = async (blob: Blob) => {
      ...
      fd.append('frame', blob, `frame-${Date.now()}.webp`);
      try {
        setAiUploading(true);
        ...
        // === AI 응답 해석 → 딴짓 모달 표시 + BE 로그 저장 ===
        // { events: {PHONE, LEFT_SEAT, DROWSY}, metrics: {phone_score?, ear?, latency_ms?}, ts }
        const now = Date.now();
        ...
        if (hit) {
          const ts: string = (json?.ts as string) ?? new Date().toISOString();
          const conf: number =
            (json?.metrics?.phone_score as number | undefined) ?? 0.9;

          // 1) 모달 열기(감지 정보 함께 전달)
          if (!isFeedbackModalOpen) {
            setDetectedActivity(hit);
            setDetectedAt(ts);
            setDetectedConf(conf);
            setIsFeedbackModalOpen(true);
          }

          // 2) BE 로그 저장(StudySession API)
